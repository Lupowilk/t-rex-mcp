...
pub struct EligibilityCheck {
    ...
    to: String,
    #[schemars(
        description = "Transfer amount as a whole number in the token's raw base units (smallest unit, no decimals applied)"
    )]
    amount: String,
}
...
pub struct IdentityCheck {
    ...
    token: String,
    #[schemars(
        description = "Holder wallet address to look up in the token's identity registry, 0x-prefixed hex"
    )]
    holder: String,
}
...
impl TRexServer {
    ...
    pub async fn read_identity_registry(
        ...
        let onchainid_check_contract = identitydetails.0.token.parse::<Address>().map_err(|e| {
            McpError::invalid_params(
                format!("Invalid token address (expected 0x-prefixed hex): {e}"),
                None,
            )
        ...
        let onchain_holder_check = identitydetails.0.holder.parse::<Address>().map_err(|e| {
            McpError::invalid_params(
                format!("Invalid holder address (expected 0x-prefixed hex): {e}"),
                None,
            )
        })?;
        ...
        if registry_identity_call == Address::ZERO {
            return Ok(CallToolResult::structured(
                json!({ "registered": false, "onchainid": null }),
            ));
        }
        Ok(CallToolResult::structured(
            json!({ "registered": true, "onchainid": registry_identity_call.to_string() }),
        ))
    }
    ...
    pub async fn list_claim_topics(
        ...
        let token_address = claimdetails.0.token.parse::<Address>().map_err(|e| {
            McpError::invalid_params(
                format!("Invalid token address (expected 0x-prefixed hex): {e}"),
                None,
            )
